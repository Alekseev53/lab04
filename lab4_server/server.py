...
def generate_text():
    ...
    try:
        ...
        model_name_or_path = 'Vikhrmodels/Vikhr-Llama-3.2-1B-Instruct'
        ...
        out = model.generate(
            input_ids, 
            attention_mask=torch.ones(input_ids.shape, dtype=torch.long),  # Установка attention_mask
            max_length=default_values["max_length"], 
            temperature=default_values["temperature"], 
            top_k=default_values["top_k"], 
            top_p=default_values["top_p"],
            num_return_sequences=default_values["num_options"], 
            no_repeat_ngram_size=default_values["no_repeat_ngram"], 
            do_sample=default_values["do_sample"], 
            pad_token_id=tokenizer.eos_token_id  # Явное указание pad_token_id
        )
        ...
        if data["model"].lower() == "llama":  
            return jsonify({
                "text": generated_list[0][17+len(data["prompt"]):]
            }), 200
        else:
            return jsonify({
